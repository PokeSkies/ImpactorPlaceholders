...
class BalanceShortStrip : PlayerPlaceholder {
    override fun handle(player: ServerPlayer, args: List<String>): GenericResult {
        ...
        return GenericResult.valid(
            ImpactorUtils.formatShort(ImpactorUtils.balance(player, currency), decimals, true)
        )
    }

    override fun ids(): List<String> = listOf("balance_short_strip")
}
