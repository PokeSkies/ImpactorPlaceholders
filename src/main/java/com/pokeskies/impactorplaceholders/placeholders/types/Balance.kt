...
class Balance : PlayerPlaceholder {
    override fun handle(player: ServerPlayer, args: List<String>): GenericResult {
        ...
        return GenericResult.valid(ImpactorUtils.formatDecimalPlaces(ImpactorUtils.balance(player, currency), decimals).toPlainString())
    }

    override fun ids(): List<String> = listOf("balance")
}
